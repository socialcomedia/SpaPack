...
//|=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-||
  ...
  void loop(){ 
    ...
    delay(2000);
//    makeClear();
    wifi(2, "Ferrara-Downstairs");
    tempTemp++;
    temperature(tempTemp);
    setTemp(tempTemp);
    cycle(cycles[cycleInt], 120); 
    cycleInt++;
    time();
    if (cycleInt > 4) cycleInt = 0;
    buttons();
  }
